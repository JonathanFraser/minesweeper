...
type alias Square =
    { covered : Covered, isMine : Bool, state : GameState }
        ...
        case value of
            ...
            Just v ->
                if v.covered == Marked then
                    table
                else if v.isMine || getUnFlaggedCount table pos /= 0 || v.covered == Uncovered then
                    uncovered
                else
...
anyTable f =
    Dict.foldl (\_ v p -> p || f v) False


allTable : (Square -> Bool) -> GameTable -> Bool
allTable f =
    Dict.foldl (\_ v p -> p && f v) True


hasHit : Square -> Bool
hasHit square =
    square.covered == Uncovered && square.isMine


isValidMark : Square -> Bool
isValidMark s =
    not (xor (s.covered == Marked) s.isMine)
...
isWon table =
    (allTable isValidMark table) && not (isLost table)
...
getCount =
    getNeighbourCount .isMine


getFlagCount : GameTable -> Pos -> Int
getFlagCount =
    getNeighbourCount (\s -> s.covered == Marked)


getUnFlaggedCount : GameTable -> Pos -> Int
getUnFlaggedCount =
    getNeighbourCount (\s -> s.isMine && (not (s.covered == Marked)))


getNeighbourCount : (Square -> Bool) -> GameTable -> Pos -> Int
getNeighbourCount f table pos =
    getNeighbours pos
...
minesLeft table =
    let
        mines =
            Dict.size (Dict.filter (\_ s -> s.isMine) table)

        flags =
            Dict.size (Dict.filter (\_ s -> s.covered == Marked) table)
    in
        if flags > mines then
            0
        else
...
nullCell : Square
nullCell =
    { covered = Unmarked, isMine = False, state = Playing }
                ...
                Just pos ->
                    let
                        new =
                            case clicktype of
                                MarkCmd ->
                                    { model | table = operate toggleMark pos model.table }

                                UncoverCmd ->
                                    { model | table = uncoverSquare pos model.table }

                                FlushCmd ->
                                    if (getFlagCount model.table pos) == (getCount model.table pos) then
                        ...
                        latch =
                            if model.state /= Playing then
                                model
                            else if isLost new.table then
                                { new | state = Lost }
                            else if isWon new.table then
                                { new | state = Won }
                            else
                                new
                    in
...
tableElement table pos =
    let
        ( attrs, str ) =
            spriteProps (getSprite table pos)
    in
...
main =
    Html.program { init = ( initModel, Random.generate NewTable (randomTable (initModel.size) 8) ), subscriptions = subscriptions, view = Html.Lazy.lazy view, update = (\msg model -> update msg model) }


view model =
    Html.table [ Attrs.style [ ( "background-color", "lightgrey" ), ( "cursor", "pointer" ) ] ]
        [ Html.tr []
            [ Html.th [ Attrs.style [ ( "width", "40px" ) ] ] [ renderMineCount model.table ]
